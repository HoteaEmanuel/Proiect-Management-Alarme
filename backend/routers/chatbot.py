...
async def send_message_to_chatbot(
    ...
    file_preserve_flags: list[bool] = Form(default=[])
):
    while len(file_preserve_flags) < len(files):
            file_preserve_flags.append(False)   
    
    raw_files = []
    for upload_file, preserve in zip(files, file_preserve_flags):
...
def edit_conversation_title(conversation_id: str, new_title: str, user_id: str = Depends(get_current_user), db: Session = Depends(get_db)):
    try:
        update_conversation_title(db=db, user_id=user_id["id"], conversation_id=conversation_id, new_title=new_title)
